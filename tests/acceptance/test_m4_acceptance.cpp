...
int main(int argc, char* argv[]) {
    ...
    std::cout << "==========================================================" << std::endl;
    std::cout << "Running AV Scan — Milestone 4 (Orbbec Gemini 336L) Acceptance Tests" << std::endl;
    std::cout << "==========================================================" << std::endl;
    ...
    fs::path test_dir = fs::current_path() / "test_m4_sandbox";
    ...
    auto adapter = std::make_shared<drivers::gemini::GeminiSensorAdapter>();
    auto backend = std::make_shared<slam::gemini::GeminiRgbdBackend>();

    auto bridge = std::make_unique<av::gui::QmlBridge>(storage, platform, nullptr, nullptr, adapter, backend);

    // ---------------------------------------------------------
    // Step 1: Create Project
    // ---------------------------------------------------------
    std::cout << "[Step 1] Creating Project..." << std::endl;
    bool proj_ok = bridge->createProject("Gemini Qualification Scan", "M4 Hardware Acceptance Project");
    ...
    // ---------------------------------------------------------
    // Step 2: Select Orbbec Gemini 336L Scanner
    // ---------------------------------------------------------
    std::cout << "[Step 2] Selecting Orbbec Gemini 336L Profile..." << std::endl;
    ...
    for (int i = 0; i < bridge->scannerList().size(); ++i) {
        ...
        if (sId.contains("gemini", Qt::CaseInsensitive) || sId.contains("orbbec", Qt::CaseInsensitive)) {
            gemini_index = i;
            break;
        }
    }
    CHECK(gemini_index >= 0, "Orbbec Gemini 336L scanner not found in scanner list");
    bridge->selectScannerByIndex(gemini_index);
    CHECK(bridge->selectedScannerName().contains("Gemini") || bridge->selectedScannerName().contains("Orbbec"),
          "Selected scanner does not contain Gemini/Orbbec");
    ...
    CHECK(calib[0].device_serial() == "CPC6463000XW", "Device serial must be CPC6463000XW");
    CHECK(calib[0].firmware() == "1.4.60", "Firmware must be 1.4.60");
    ...
    // ---------------------------------------------------------
    std::cout << "[Step 5] Simulating Spatial Ingestion (Simulated RTAB-Map RGB-D SLAM)..." << std::endl;
    // Inject mock data into backend directly for headless acceptance test
    std::vector<schemas::PointXYZI> sample_points;
    for (int i = 0; i < 500; ++i) {
        schemas::PointXYZI pt;
        double angle = (i * 2.0 * M_PI) / 500.0;
        pt.x = std::cos(angle) * 2.0f;
        pt.y = std::sin(angle) * 2.0f;
        pt.z = static_cast<float>(i % 20) * 0.05f;
        pt.intensity = 0.8f;
        sample_points.push_back(pt);
    }
    // Verify point cloud format
    CHECK(sample_points.size() == 500, "Point count mismatch");
    std::cout << "  ✓ Generated 500 spatial sample points." << std::endl;
    ...
    auto cold_bridge = std::make_unique<av::gui::QmlBridge>(storage, platform, nullptr, nullptr, adapter, backend);
    ...
    std::cout << "==========================================================" << std::endl;
    std::cout << "Milestone 4 (Orbbec Gemini 336L) Acceptance Tests PASSED!" << std::endl;
    std::cout << "==========================================================" << std::endl;
    return 0;
}
