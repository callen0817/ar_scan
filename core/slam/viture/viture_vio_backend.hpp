...
class VitureVioBackend : public interfaces::ISlamBackend {
public:
    explicit VitureVioBackend(std::shared_ptr<drivers::viture::VitureIpcClient> ipc_client = nullptr);
    ~VitureVioBackend() override;
...
private:
    std::shared_ptr<drivers::viture::VitureIpcClient> ipc_client_;
