...
class AirySensorAdapter : public interfaces::ISensorAdapter {
public:
    explicit AirySensorAdapter(std::shared_ptr<AiryIpcClient> ipc_client = nullptr);
    ~AirySensorAdapter() override;
    ...
    // Helper for SLAM backend sharing
    std::shared_ptr<AiryIpcClient> getIpcClient() const { return ipc_client_; }

private:
    std::shared_ptr<AiryIpcClient> ipc_client_;
